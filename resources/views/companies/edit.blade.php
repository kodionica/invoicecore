<x-layouts.layout>
    ...
    <x-forms.form method="POST" action="{{ route('client.edit', $client) }}" class="d-flex flex-column justify-content-start gap-3">
        @method('PATCH')
    ...
    <form action="{{ route('client.destroy', $client) }}" method="POST" class="sr-only" id="delete-form">
        @csrf
        @method('DELETE')
    </form>
</x-layouts.layout>
